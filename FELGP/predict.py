...
# x_test = np.load(dataSetName+'_train_data.npy')
# y_test = np.load(dataSetName+'_train_label.npy')
# x_train = np.load(dataSetName+'_test_data.npy')
# y_train = np.load(dataSetName+'_test_label.npy')

x2_test = x_test[-100:]
y2_test = y_test[-100:]

# x_test = np.load('new_test_data.npy')
# y_test = np.load('new_test_label.npy')
# x_train = np.load(dataSetName+'_test_data.npy')
# y_train = np.load(dataSetName+'_test_label.npy')
...
def evalTest(toolbox, individual, trainData, trainLabel, test, testL):
    ...
    print(output.shape)
    score = np.diff(output)
    score = score.reshape(30)
    y_predict = np.argmax(output, axis=1)
    accuracy = 100*np.sum(y_predict==testL)/len(testL)
    return score, y_predict, testL,accuracy

if __name__ == "__main__":

    txtpath = '42_Final_Result_son_axdwiselect.txt'
    ...
    score, prediction, testL, testResults = evalTest(toolbox, info, x_train, y_train, x_test, y_test)
    testTime = time.process_time() - beginTime

    testcsv = pd.DataFrame({
        'testL':testL,
        'FELGP_axdwi_ori':prediction,
        'score':score,
        'testtime':testTime
    })
    # testcsv.to_csv('FELGP_axdwi_ori_pre.csv')
    print(testResults)
    print(score)
    print(prediction)
    print(testL)
