...
def evalTest(toolbox, individual, trainData, trainLabel, test, testL):
    ...
    return score, prediction, testL, accuracy


if __name__ == "__main__":
    txtpath = 'RESULT5_WITH_SAME_PARAMETERS_0/42_Final_Results_on_axdwioriselect.txt'
    ...
    testcsv = pd.DataFrame({
        'testL':testL,
        'FlexGP_axdwi_roi':prediction,
        'score':score,
        'testtime':testTime
    })
    testcsv.to_csv('FlexGP_axdwi_ori_pre.csv')
    print(testResults)
    print(score)
    print(prediction)
    print(testL)
